                                        ...
                                        <div class="table-scrollable table-scrollable-borderless">
                                            ...
                                            <table v-else class="table-striped table-checkable table table-hover table-bordered admin">
                                                ...
                                                <tr v-for="user in data.data" :class="[$index % 2 == 0 ? 'odd' : 'even']">
                                                    ...
                                                    <td>
                                                        <label>
                                                            <input type="radio" v-model="user.affiliate.type" :value="0" @change.prevent="onChange({{auth()->user()->id}})">
                                                            Non-approved
                                                        </label>&nbsp;&nbsp;
                                                        <label>
                                                            <input type="radio" v-model="user.affiliate.type" :value="1" @change.prevent="onChange({{auth()->user()->id}})">
                                                            Percent
                                                        </label>&nbsp;&nbsp;
                                                        <label>
                                                            <input type="radio" v-model="user.affiliate.type" :value="2" @change.prevent="onChange({{auth()->user()->id}})">
                                                            Cash
                                                        </label>
                                                    </td>
                                                    <td><input type="text" v-model="formUpdate.values" class="form-control" placeholder="value"> </td>
